...
export function StudentDashboard() {
  ...
  const displayName = student?.user?.name || name || "Student";
  ...
  return (
    <div className="mx-auto max-w-[1400px]">
      ...
      <div className="mt-8 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {[
          { label: "st_att_label" as const, value: `${attendancePercent}%` },
          { label: "st_due_label" as const, value: null },
          { label: "st_exam_label" as const, value: "0" },
          { label: "st_unread" as const, value: String(unread) },
        ].map((card) => (
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <article className="card-surface p-5">
          ...
          {courseRows.length === 0 ? (
            <p className="mt-4 font-sans text-sm text-zinc-400">No enrolled courses yet.</p>
          ) : (
        ...
        <article className="card-surface p-5">
          ...
          {board.length === 0 ? (
            <p className="mt-4 font-sans text-sm text-zinc-400">No notices yet.</p>
          ) : (
